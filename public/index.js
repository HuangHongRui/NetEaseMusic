...
$(function () {
	...
	$('.siteNav').on('tabChange', function (e, index) {
		var $li = $('.tabContent >li').eq(index);
		if ($li.attr('data-downloaded') === 'yeah') return;
		if (index === 1) {
			$.get('../src/js/lib/page2.json').then(function (response) {
				$li.text(response.text);
				$li.attr('data-downloaded', 'yeah');
			});
		} else if (index === 2) {
			//函数节流
			$.get('../src/js/lib/page3.json').then(function (response) {
				$li.text(response.text);
				$li.attr('data-downloaded', 'yeah');
			});
		}
	...
	$('input#searchMusic').on('input', function (e) {
		...
		clockLock = setTimeout(function () {
			search(value).then(function (result) {
				if (result.length !== 0) {
					$('#output').text(result.map(function (r) {
						return r.name;
					}).join(','));
				} else {
					$('#output').text('没有');
